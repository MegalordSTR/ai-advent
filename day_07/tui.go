...
func newAgentSelectModel(agents []agent.Agent, client *deepseek.DeepSeekClient, db *sql.DB) agentSelectModel {
	return agentSelectModel{agents: agents, client: client, db: db}
}
	...
	case tea.KeyMsg:
		...
		case "enter", " ":
			if len(m.agents) > 0 {
				...
				chatModel := newChatModel(selected, m.client, m.db)
...
func newChatModel(agent agent.Agent, client *deepseek.DeepSeekClient, db *sql.DB) chatModel {
	ta := textarea.New()
	...
	if db != nil {
		loaded, err := storage.LoadMessages(db, agent.Name)
		if err != nil {
			// Log error but continue without history
			log.Printf("failed to load messages for agent %s: %v", agent.Name, err)
		} else {
			messages = loaded
		}
	}
...
// saveMessage saves a message to the database (logs errors silently).
func (m chatModel) saveMessage(role, content string) {
	if m.db == nil {
		return
	}
	if err := storage.SaveMessage(m.db, m.agent.Name, role, content); err != nil {
		log.Printf("failed to save message: %v", err)
	}
	// Keep only recent messages in database (limit to 2*maxMessages)
	if err := storage.PruneMessages(m.db, m.agent.Name, maxMessages*2); err != nil {
		log.Printf("failed to prune messages: %v", err)
	}
}
	...
	if m.waiting {
		...
		case chatResponseMsg:
			...
			if msg.err != nil {
				...
				// Save to database
				m.saveMessage("assistant", msg.content)
				m.status = "Ready"
				m.textarea.Reset()
